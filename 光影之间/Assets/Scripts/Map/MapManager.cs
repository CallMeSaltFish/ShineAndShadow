...
public class MapManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Time.timeScale = 0;
            isEverEnabled = rotatePlayer.isActiveAndEnabled;
            if(rotatePlayer.isActiveAndEnabled)
            {
                rotatePlayer.enabled = false;
            }
            rotatePlayer.enabled = false;
        ...
        if (chapter != 0)
        {
            if (Input.GetMouseButtonDown(2) && Camera.main.transform.position.x - playerTransform.position.x >= 5.2f)
            {
                ...
                {
                    GameObject[] goes = GameObject.FindGameObjectsWithTag("BackGround");
                    foreach(GameObject go in goes)
                    {
                        MapMover temp = go.GetComponent<MapMover>();
                        if(temp != null)
                        {
        ...
        if (BossTag.IsEat == true || playerMove.IsDead == true)
        {
            ...
            //停住人和怪兽
            mapMaker.enabled = false;
            mapMakerIn4.enabled = false;
            playerMove.enabled = false;
            BossB.enabled = false;
            BossW.enabled = false;
            //屏幕逐渐变暗
    ...
    public void ChangeToNextChapter()
    {
        ...
        switch (chapter)
        {
            ...
            case 3:
                ...
                playerAnimator.SetBool("isStop", false);//情况特殊，需手动设置
                break;
            case 4:
                ...
                BossBT.position = new Vector3(-9.7f, 0.51f, -3f);
                BossWT.position = new Vector3(-9.7f, -2.98f, -3f);
                break;
        }
    ...
    public void ContinueGame()
    {
        ...
        rotatePlayer.enabled = isEverEnabled;
        playerMove.enabled = true;
        isPause = false;
        Time.timeScale = 1;
    }
